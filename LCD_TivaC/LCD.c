...
//private recursive definition
void rec_OUD(uint32_t value){
...
//private recursive definition
void recOUF(uint32_t value, uint32_t flag){

      int digit;
      //Least significant digit value
      digit = value%10;
      //remaining RHS of the number
      value = (value - digit)/10;
      if(value != 0 ){
          recOUF(value,flag+1);
          if(flag == 0 ){
          ...
          //recursive calling can isolate LSB from LHS
          // use position flag to track recursive calls and know when to put '.' in the right place
          LCD_OutChar(0x30 + digit);
      }
...
//public function
void LCD_OutUFix(uint32_t value) {

      if(value < 10){
        //case handle if number is small and needs to be zero-padded decimal
        LCD_OutChar(0x30 + 0);
        LCD_OutChar(0x2e);
        LCD_OutChar(0x30 + value);
        return;
          }

      if(value > 10000 ){
        //case handle if number is too big for range
        LCD_OutChar('*');
        LCD_OutChar(0x2e);
        LCD_OutChar('*');
        return;
